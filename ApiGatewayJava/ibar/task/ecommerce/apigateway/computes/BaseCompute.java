...
import ibar.task.ecommerce.apigateway.exceptions.CommonException;
import ibar.task.ecommerce.apigateway.exceptions.UknownException;
import ibar.task.ecommerce.apigateway.utils.ApplicationContext;
...
public abstract class BaseCompute extends JavaComputeNode {
	...
	public abstract class BaseJavaComputeEvaluator extends JavaComputeEvaluator {
		...
		public void init() throws Exception {
			...
			assignMessageId();
			context.getObjectMapper().setVisibility(PropertyAccessor.ALL,
					Visibility.NONE);
			context.getObjectMapper().setVisibility(PropertyAccessor.FIELD,
					Visibility.ANY);
		...
		public void handleException(BaseJavaComputeEvaluator evaluator,
				Exception e) throws Exception {
			logger.error(messageId, "--------EXCEPTION OCCURED------------");
			logger.error(messageId, context.getStackTraceReceiver()
					.getStackTrace(e));
			context.getObjectMapper().setSerializationInclusion(
					Include.NON_NULL);
			...
			String outJson = context.getObjectMapper().writeValueAsString(
					localError);
			...
			headerElement.createElementAsLastChild(MbElement.TYPE_NAME_VALUE,
					"X-Original-HTTP-Status-Code", localError.getStatusCode());
			propagateAlt();
		}

		public String generateRequestLog() throws Exception {
			MbElement headerElement = outputRoot().getMbElement()
					.getFirstElementByPath("HTTPInputHeader");
			HashMap<String, String> headers = new HashMap<>();
			if (headerElement != null) {
				NodeList list = headerElement.getDOMNode().getChildNodes();
				for (int i = 0; i < list.getLength(); i++) {
					headers.put(list.item(i).getNodeName(), list.item(i)
							.getTextContent());
				}
			}
			String serviceUrl = localEnvironment().getAsString(
					"Destination/HTTP/RequestURL");
			String json = "";
			Object contentNode = outputRoot().get("BLOB/BLOB");
			if (contentNode != null) {
				json = new String((byte[]) outputRoot().get("BLOB/BLOB"));
			}
			String method = localEnvironment().getAsString(
					"Destination/HTTP/RequestLine/Method");
			return "url: "
					+ serviceUrl
					+ "\nmethod: "
					+ method
